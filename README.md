...
- **موتور:** Godot 4.7
- **وضعیت فعلی:** فازهای `۰` تا `۲` و `۴` (World Identity) انجام شده. فاز `۳` (Survival Loop) در حال تکمیل است — مانده: Save واقعی + گزارش تست کاربر.
...
| ۳ | **Survival Loop** — تعامل (E)، برداشتن/مصرف آب و غذا، تخلیه‌ی hunger/thirst، مرگ، HUD، **Save واقعی** | 🟡 در حال تکمیل (مانده: Save واقعی) |
| ۴ | **World Identity** — بلوک شهری، تکسچرها، اتمسفر گرگ‌ومیش، چراغ‌قوه | ✅ انجام شد (۰.۴.۰) |
| ۵ | Threat/Enemy AI | ⬜ بعد از تکمیل ۳ |
| ۶ | Inventory/Crafting عمیق | ⬜ بعد از ۵ |

**اولویت فعلی:** تکمیل فاز ۳ (Survival Loop) — نه Inventory.
...
- [ ] **Save واقعی/Load** — SaveManager وجود دارد ولی به گیم‌پلی وصل نیست (کلید/رویداد save-load ندارد)
- [ ] گزارش تست کاربر روی صحنه‌ی جدید (موارد بصری/کالیژن)
...
5. به سمت اشیاء (بطری آب، کنسرو، جعبه کمک‌ها، سوئیچ برق) نگاه کن تا نشانگر `[E]` ظاهر شود.
6. کلید `E` را بزن تا آب بنوشی، غذا بخوری یا ژنراتور را روشن/خاموش کنی.
7. نوارهای وضعیت سلامت، استامینا، گرسنگی و تشنگی را در پایین سمت چپ به صورت زنده مشاهده کن.
8. از تقاطع به سمت ساختمان‌ها برو؛ پنجره‌های روشن و چراغ‌های اضطراری در گرگ‌ومیش می‌درخشند.
