## تست کارکردی headless: prop غیرتعاملی «تابلوی شکسته» (broken_sign.tscn)
...
const PROP_PATH: String = "res://entities/decor/broken_sign.tscn"
const LEVEL_PATH: String = "res://levels/test_level.tscn"
const PROP_NODE_NAME: String = "BrokenSign"
...
func _run() -> void:
	...
	_check(packed != null, "broken_sign.tscn لود می‌شود")
	if packed == null:
		return
	...
	_check(_count_meshes(root3d) >= 2, "prop حداقل ۲ مش دارد (تیر + تابلو + تکه‌ی ریخته)")
	prop.free()
	...
	_check(placed != null, "BrokenSign در بلوک شهری instance شده است")
	if placed == null:
		return
	_check(placed is Node3D, "instance داخل سطح یک Node3D است")
	var placed3d: Node3D = placed as Node3D
	_check(absf(placed3d.global_position.x - (-2.0)) < 0.01, "BrokenSign در x=-2 قرار دارد")
	_check(absf(placed3d.global_position.z - 12.0) < 0.01, "BrokenSign در z=12 قرار دارد")
	_check(absf(placed3d.global_position.y - 0.0) < 0.01, "BrokenSign روی زمین (y=0) است")
...
func _finish() -> void:
	...
	if failures == 0:
		print("ALL TESTS PASSED (prop broken sign)")
		quit(0)
	else:
