## تست کارکردی headless: prop غیرتعاملی «دوچرخه‌ی زنگ‌زده» (rusted_bicycle.tscn)
...
##   godot --headless --path . -s res://tests/prop_rusted_bicycle_test.gd
##
## کد خروجی ۰ = همه‌ی تست‌ها پاس، ۱ = شکست.
extends SceneTree

const PROP_PATH: String = "res://entities/decor/rusted_bicycle.tscn"
const LEVEL_PATH: String = "res://levels/test_level.tscn"
const PROP_NODE_NAME: String = "RustedBicycle"
...
func _run() -> void:
	...
	_check(packed != null, "rusted_bicycle.tscn لود می‌شود")
	if packed == null:
		return
	...
	_check(_count_meshes(root3d) >= 3, "prop حداقل ۳ مش دارد (چرخ‌ها + شاسی)")
	prop.free()
	...
	_check(placed != null, "RustedBicycle در بلوک شهری instance شده است")
	if placed == null:
		return
	_check(placed is Node3D, "instance داخل سطح یک Node3D است")
	var placed3d: Node3D = placed as Node3D
	_check(absf(placed3d.global_position.x - 35.0) < 0.01, "RustedBicycle در x=35 قرار دارد")
	_check(absf(placed3d.global_position.z - 5.0) < 0.01, "RustedBicycle در z=5 قرار دارد")
	_check(absf(placed3d.global_position.y - 0.0) < 0.01, "RustedBicycle روی زمین (y=0) است")
...
func _finish() -> void:
	...
	if failures == 0:
		print("ALL TESTS PASSED (prop rusted bicycle)")
		quit(0)
	else:
