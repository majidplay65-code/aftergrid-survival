## تست کارکردی headless: prop غیرتعاملی «چادر پاره» (torn_tent.tscn)
...
##   godot --headless --path . -s res://tests/prop_torn_tent_test.gd
##
## کد خروجی ۰ = همه‌ی تست‌ها پاس، ۱ = شکست.
extends SceneTree

const PROP_PATH: String = "res://entities/decor/torn_tent.tscn"
const LEVEL_PATH: String = "res://levels/test_level.tscn"
const PROP_NODE_NAME: String = "TornTent"
...
func _run() -> void:
	...
	_check(packed != null, "torn_tent.tscn لود می‌شود")
	if packed == null:
		return
	...
	_check(_count_meshes(root3d) >= 3, "prop حداقل ۳ مش دارد (کف + پنل‌ها + تکه‌ی پارگی)")
	prop.free()
	...
	_check(placed != null, "TornTent در بلوک شهری instance شده است")
	if placed == null:
		return
	_check(placed is Node3D, "instance داخل سطح یک Node3D است")
	var placed3d: Node3D = placed as Node3D
	_check(absf(placed3d.global_position.x - (-32.0)) < 0.01, "TornTent در x=-32 قرار دارد")
	_check(absf(placed3d.global_position.z - (-18.0)) < 0.01, "TornTent در z=-18 قرار دارد")
	_check(absf(placed3d.global_position.y - 0.0) < 0.01, "TornTent روی زمین (y=0) است")
...
func _finish() -> void:
	...
	if failures == 0:
		print("ALL TESTS PASSED (prop torn tent)")
		quit(0)
	else:
