## تست کارکردی headless: دستور ساخت جدید «طناب» (craft_rope.tres)
## - بارگذاری .tres و راستی‌آزمایی مقادیر RecipeData
## - حضور دستور در کاتالوگ اصلی
## - ساخت واقعی از طریق CraftingSystem: موفق با ۳ پارچه، شکست با مواد ناکافی
## - بررسی سیگنال‌های EventBus (item_crafted / craft_failed)
##
## نحوه‌ی اجرا (از ریشه‌ی پروژه):
##   godot --headless --path . -s res://tests/recipe_craft_rope_test.gd
##
## کد خروجی ۰ = همه‌ی تست‌ها پاس، ۱ = شکست.
extends SceneTree

const RECIPE_PATH: String = "res://resources/recipes/craft_rope.tres"
...
func _run() -> void:
	...
	_check(raw != null, "craft_rope.tres لود می‌شود")
	if raw == null:
		return
	_check(raw is RecipeData, "craft_rope.tres از نوع RecipeData است")
	var recipe: RecipeData = raw as RecipeData
	_check(recipe.recipe_id == &"craft_rope", "craft_rope شناسه‌ی درست دارد")
	_check(recipe.recipe_name == "طناب", "craft_rope نام فارسی درست دارد")
	_check(recipe.result_item_id == &"rope", "craft_rope خروجی rope دارد")
	_check(recipe.result_amount == 1, "craft_rope تعداد خروجی ۱ دارد")
	_check(recipe.ingredients.size() == 1, "craft_rope یک ماده لازم دارد")
	_check(recipe.ingredients.has(&"cloth") and int(recipe.ingredients[&"cloth"]) == 3,
			"craft_rope به ۳ پارچه نیاز دارد")
	_check(recipe.craft_time == 2.0, "craft_rope زمان ساخت ۲ ثانیه دارد")
	...
	if cat == null:
		return
	_check(cat.has_recipe(&"craft_rope"), "craft_rope در کاتالوگ اصلی ثبت شده است")
	_check(cat.get_recipe(&"craft_rope") == recipe, "نمونه‌ی کاتالوگ همان craft_rope.tres است")

	# ساخت واقعی از طریق CraftingSystem autoload روی یک Inventory جدا.
	var inv: Inventory = Inventory.new()
	inv.catalog = cat
	inv.add_item(&"cloth", 3)
	_check(crafting.can_craft(recipe, inv), "با ۳ پارچه، can_craft true است")
	_check(crafting.craft(recipe, inv), "ساخت طناب موفق است")
	_check(inv.count_item(&"cloth") == 0, "بعد از ساخت، هر ۳ پارچه مصرف شد")
	_check(inv.count_item(&"rope") == 1, "بعد از ساخت، ۱ طناب به اینونتوری آمد")
	_check(crafted_ids.size() == 1 and crafted_ids[0] == &"craft_rope",
	...
	_check(not crafting.can_craft(recipe, empty_inv), "بدون پارچه، can_craft false است")
...
func _finish() -> void:
	...
	if failures == 0:
		print("ALL TESTS PASSED (recipe craft rope)")
		quit(0)
	else:
