## تست کارکردی headless: دستور ساخت جدید «فیلتر هوا» (craft_air_filter.tres)
## - بارگذاری .tres و راستی‌آزمایی مقادیر RecipeData (دو ماده: ۲ قراضه + ۱ پارچه)
...
##   godot --headless --path . -s res://tests/recipe_craft_air_filter_test.gd
##
## کد خروجی ۰ = همه‌ی تست‌ها پاس، ۱ = شکست.
extends SceneTree

const RECIPE_PATH: String = "res://resources/recipes/craft_air_filter.tres"
...
func _run() -> void:
	...
	_check(raw != null, "craft_air_filter.tres لود می‌شود")
	if raw == null:
		return
	_check(raw is RecipeData, "craft_air_filter.tres از نوع RecipeData است")
	var recipe: RecipeData = raw as RecipeData
	_check(recipe.recipe_id == &"craft_air_filter", "craft_air_filter شناسه‌ی درست دارد")
	_check(recipe.recipe_name == "فیلتر هوا", "craft_air_filter نام فارسی درست دارد")
	_check(recipe.result_item_id == &"air_filter", "craft_air_filter خروجی air_filter دارد")
	_check(recipe.result_amount == 1, "craft_air_filter تعداد خروجی ۱ دارد")
	_check(recipe.ingredients.size() == 2, "craft_air_filter دو ماده لازم دارد")
	_check(recipe.ingredients.has(&"scrap_metal") and int(recipe.ingredients[&"scrap_metal"]) == 2,
			"craft_air_filter به ۲ قراضه نیاز دارد")
	_check(recipe.ingredients.has(&"cloth") and int(recipe.ingredients[&"cloth"]) == 1,
			"craft_air_filter به ۱ پارچه نیاز دارد")
	_check(recipe.craft_time == 3.0, "craft_air_filter زمان ساخت ۳ ثانیه دارد")
	...
	if cat == null:
		return
	_check(cat.has_recipe(&"craft_air_filter"), "craft_air_filter در کاتالوگ اصلی ثبت شده است")
	_check(cat.get_recipe(&"craft_air_filter") == recipe, "نمونه‌ی کاتالوگ همان craft_air_filter.tres است")
	...
	_check(crafting.can_craft(recipe, inv), "با ۲ قراضه + ۱ پارچه، can_craft true است")
	_check(crafting.craft(recipe, inv), "ساخت فیلتر هوا موفق است")
	_check(inv.count_item(&"scrap_metal") == 0, "بعد از ساخت، قراضه‌ها مصرف شدند")
	_check(inv.count_item(&"cloth") == 0, "بعد از ساخت، پارچه مصرف شد")
	_check(inv.count_item(&"air_filter") == 1, "بعد از ساخت، ۱ فیلتر هوا به اینونتوری آمد")
	_check(crafted_ids.size() == 1 and crafted_ids[0] == &"craft_air_filter",
...
func _finish() -> void:
	...
	if failures == 0:
		print("ALL TESTS PASSED (recipe craft air filter)")
		quit(0)
	else:
