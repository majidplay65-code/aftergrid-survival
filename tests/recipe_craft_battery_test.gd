## تست کارکردی headless: دستور ساخت جدید «باتری» (craft_battery.tres)
## - بارگذاری .tres و راستی‌آزمایی مقادیر RecipeData (دو ماده: ۳ قراضه + ۱ پارچه)
...
##   godot --headless --path . -s res://tests/recipe_craft_battery_test.gd
##
## کد خروجی ۰ = همه‌ی تست‌ها پاس، ۱ = شکست.
extends SceneTree

const RECIPE_PATH: String = "res://resources/recipes/craft_battery.tres"
...
func _run() -> void:
	...
	_check(raw != null, "craft_battery.tres لود می‌شود")
	if raw == null:
		return
	_check(raw is RecipeData, "craft_battery.tres از نوع RecipeData است")
	var recipe: RecipeData = raw as RecipeData
	_check(recipe.recipe_id == &"craft_battery", "craft_battery شناسه‌ی درست دارد")
	_check(recipe.recipe_name == "باتری", "craft_battery نام فارسی درست دارد")
	_check(recipe.result_item_id == &"battery", "craft_battery خروجی battery دارد")
	_check(recipe.result_amount == 1, "craft_battery تعداد خروجی ۱ دارد")
	_check(recipe.ingredients.size() == 2, "craft_battery دو ماده لازم دارد")
	_check(recipe.ingredients.has(&"scrap_metal") and int(recipe.ingredients[&"scrap_metal"]) == 3,
			"craft_battery به ۳ قراضه نیاز دارد")
	_check(recipe.ingredients.has(&"cloth") and int(recipe.ingredients[&"cloth"]) == 1,
			"craft_battery به ۱ پارچه نیاز دارد")
	_check(recipe.craft_time == 5.0, "craft_battery زمان ساخت ۵ ثانیه دارد")
	...
	if cat == null:
		return
	_check(cat.has_recipe(&"craft_battery"), "craft_battery در کاتالوگ اصلی ثبت شده است")
	_check(cat.get_recipe(&"craft_battery") == recipe, "نمونه‌ی کاتالوگ همان craft_battery.tres است")
	...
	_check(crafting.can_craft(recipe, inv), "با ۳ قراضه + ۱ پارچه، can_craft true است")
	_check(crafting.craft(recipe, inv), "ساخت باتری موفق است")
	...
	_check(inv.count_item(&"battery") == 1, "بعد از ساخت، ۱ باتری به اینونتوری آمد")
	_check(crafted_ids.size() == 1 and crafted_ids[0] == &"craft_battery",
...
func _finish() -> void:
	...
	if failures == 0:
		print("ALL TESTS PASSED (recipe craft battery)")
		quit(0)
	else:
